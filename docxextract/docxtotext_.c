	...
	unzFile uf;
{
	...
	for( i = 0; i < gi.number_entry; i++ )
	{
		...
		printf( " %3lu%%  %2.2lu-%2.2lu-%2.2lu  %2.2lu:%2.2lu  %8.8lx   %s\n", ratio, (uLong) file_info.tmu_date.tm_mon + 1, (uLong) file_info.tmu_date.tm_mday,
				(uLong) file_info.tmu_date.tm_year % 100, (uLong) file_info.tmu_date.tm_hour, (uLong) file_info.tmu_date.tm_min, (uLong) file_info.crc,
				filename_inzip );
...
int textconv_docx ( char * path, char *d_name, char * prefix, int bTextSearchMode, char * filteredbuff, char * buff, int fdout, size_t tStart, size_t tEnd )
{
	long txtbufflen;
...
int docx_main ( int argc, char ** argv )
{
	...
	if ( popt.bTextSearchMode == 1 )
		tempwritebytes2channel = textconv_docx( path, NULL, prefix, popt.bTextSearchMode, filteredbuff, buff, fdout, popt.tStart, popt.tEnd );
	else
	{
		dir = opendir( path );
		...
		if ( Mode != single_operation )
		{
			while(( entry = readdir( dir ) ))
			{
				...
				{
					tempwritebytes2channel = textconv_docx( path, entry->d_name, prefix, popt.bTextSearchMode, filteredbuff, buff, fdout, popt.tStart,
							popt.tEnd );
		...
		else
		{
			...
			tempwritebytes2channel = textconv_docx( "", FILESENDER_SINGLE_MODE_OUTPUT_FILE, prefix, popt.bTextSearchMode, filteredbuff, buff, fdout,
					popt.tStart, popt.tEnd );
			totalbyteswrite2text += tempwritebytes2channel;
		}
		close( fdout );
	}
