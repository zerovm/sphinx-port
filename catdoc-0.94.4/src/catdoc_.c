...
/**************************************************************/
int catdoc_main(int argc, char *argv[]) {
	FILE *f;
	int c,i;
	char *tempname;
	...
	file_out_name = (char *) malloc ( strlen ( argv[1] ) + 20 );
	sprintf( file_out_name, "%s.tmp", argv[1]);
	...
	if (!isatty(fileno(stdout))) {
		...
		if (output_buffer) {
			output_file = fopen ( file_out_name, "w" );
			if  (setvbuf(output_file ,output_buffer,_IOFBF,FILE_BUFFER)) {
