...
/************************************************************************/
#define MBCS_MAX 1024
struct mbcs_map {
	int mbcs;
	unsigned short unicode;
};

static struct mbcs_map *mbcs_charset = NULL;
static int mbcs_count;
int to_unicode (short int *charset, int c) {
	if ((c > 0) && (c <= 255))
		return charset[c];
	else {
		int i;
		for (i = 0; i < mbcs_count; i++) {
			if (mbcs_charset[i].mbcs == c)
				return mbcs_charset[i].unicode;
		}
	}
	return -1;
}
...
/* additional MBCS_MAX for mbcs mapping from charset to unicode         */
/************************************************************************/
short int * read_charset(const char *filename) {
	...
	if (!mbcs_charset)
		mbcs_charset=calloc(sizeof(struct mbcs_map),MBCS_MAX);
	int c;
	long int uc;
	...
	while (!feof(f)) {
		if (fscanf(f,"%i %li",&c,&uc)==2) {
			// store first MBCS_MAX mbcs chars
			if ((c >= 0x8000) && (c <= 0xFFFF)) {
				if (reset_mcbs) {
					reset_mcbs = 0;
					mbcs_count = 0;
				}
				if (mbcs_count < MBCS_MAX) {
					mbcs_charset[mbcs_count].mbcs = c;
					mbcs_charset[mbcs_count].unicode = uc;
					mbcs_count++;
				}
			} else
//			if (c<0||c>255||uc<0||(uc>0xFEFE && uc!=0xFFFE)) {
// shiftjis has values > 0xFEFE that are not FFFE
			if (c<0||c>255||uc<0) {
