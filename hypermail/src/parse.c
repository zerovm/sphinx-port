...
typedef enum
{
	ENCODE_NORMAL, ENCODE_QP, /* quoted printable */
...
{
	if ( inlist( set_ignore_types, "$NONPLAIN" ) && !textcontent( type ) && strncasecmp( type, "multipart/", 10 ) )
		return 1;
	if ( inlist( set_ignore_types, "$BINARY" ) && !textcontent( type ) && strcasecmp( type, "text/html" ) && strncasecmp( type, "multipart/", 10 ) )
		return 1;
...
int preferedcontent ( int *current_weight, char *type, int decode )
{
	...
	else
	{
		...
		if ( weight == -1 )
		{ /* not known to be good, use weaker evidence */
...
char *safe_filename ( char *name )
{
	...
	for( sp = name, np = name; *np && *np != '\n'; )
	{
		...
		if ( ( ( *np >= 'a' && *np <= 'z' ) || ( *np >= '0' && *np <= '9' ) || ( *np >= 'A' && *np <= 'Z' ) || ( *np == '-' ) || ( *np == '.' )
				|| ( *np == ':' ) || ( *np == '_' ) ) && !( set_unsafe_chars && strchr( set_unsafe_chars, *np ) ) )
		{
			*sp = *np;
		}
		else
			/* Need to replace the character with a safe one */
			*sp = REPLACEMENT_CHAR;
		sp++;
		np++;
	}
...
static void create_attachname ( char *attachname, int max_len )
{
	int i, max_i;
	char suffix[8];
...
void crossindex ( void )
{
	...
	while(num <= max_msgnum)
	{
		...
		status = hashreplynumlookup( email->msgnum, email->inreplyto, email->subject, &maybereply );
		if ( status != -1 )
		{
...
#ifdef FASTREPLYCODE
					replylist = addreply2( replylist, email2, email, maybereply, &replylist_end );
#else
...
void crossindexthread2 ( int num )
{
	...
	{
		char errmsg[512];
		snprintf( errmsg, sizeof( errmsg ), "internal error crossindexthread2 %d", num );
		progerr( errmsg );
	}
	...
	for( rp = ep->replylist; rp != NULL; rp = rp->next )
	{
		if ( !( rp->data->flags & USED_THREAD ) )
		{
			...
			if ( 0 )
				fprintf( stderr, "add thread.b %d %d %d\n", num, rp->data->msgnum, rp->msgnum );
			threadlist = addreply( threadlist, num, rp->data, 0, &threadlist_end );
...
void crossindexthread1 ( struct header *hp )
{
	...
	if ( hp )
	{
		 ...
		 */
		if ( !isreply && !wasprinted( printedthreadlist, hp->data->msgnum ) && !( hp->data->flags & USED_THREAD ) )
		{
			hp->data->flags |= USED_THREAD;
			threadlist = addreply( threadlist, hp->data->msgnum, hp->data, 0, &threadlist_end );
...
char *getfromdate ( char *line )
{
	...
	for( i = 0; days[i] != NULL && ( ( c = strstr( line, days[i] ) ) == NULL ); i++ )
		;
	if ( days[i] == NULL )
		tmpdate[0] = '\0';
	else
...
extract_rfc2047_content ( char *iptr )
{
	...
	ptr = strchr( iptr + 2, '?' );
	ptr++;
	if ( ( *ptr == 'Q' || *ptr == 'q' || *ptr == 'B' || *ptr == 'b' ) && *( ptr + 1 ) == '?' )
	{
...
static char *mdecodeRFC2047 ( char *string, int length, char *charsetsave )
{
	...
	while(*iptr)
	{
		if ( !strncmp( iptr, "=?", 2 ) && ( 3 == sscanf( iptr + 2, "%128[^?]?%32[^?]?%128[^ ]", charset, encoding, dummy ) ) )
		{
			...
			if ( !strncmp( iptr, "=?", 2 ) && ( 3 == sscanf( iptr + 2, "%128[^?]?%32[^?]?%128[^?]?", charset, encoding, dummy ) ) && ( blurb =
					extract_rfc2047_content( iptr ) ) )
			{
				free( blurb );
...
static char * mdecodeQP ( FILE *file, char *input, char **result, int *length, FILE *fpo )
{
	int outcount = 0;
			...
			if ( dir )
			{
				...
				while(( entry = readdir( dir ) ))
				{
					if ( !strcmp( ".", entry->d_name ) || !strcmp( "..", entry->d_name ) )
						continue;
					trio_asprintf( &filename, "%s%c%s", realdir, PATH_SEPARATOR, entry->d_name );
					if ( set_showprogress )
						fprintf( stderr, "\nWe delete %s\n", filename );
					unlink( filename );
					free( filename );
...
static int do_uudecode ( FILE *fp, char *line, char *line_buf, struct Push *raw_text_buf, FILE *fpo )
{
	struct Push pbuf;
	char *p2;
	INIT_PUSH( pbuf );
...
static void write_txt_file ( struct emailinfo *emp, struct Push *raw_text_buf )
{
	...
	if ( ( !emp->is_deleted || ( ( emp->is_deleted & ( FILTERED_DELETE | FILTERED_OLD | FILTERED_NEW ) ) && set_delete_level > 2 )
			|| ( emp->is_deleted == FILTERED_EXPIRE && set_delete_level == 2 ) ) && ( set_overwrite || !isfile( txt_filename ) ) )
	{
...
#define IO_SIZE_BUF 0x10000
	...
	{
		snprintf( errmsg, sizeof( errmsg ), "%s \"%s\".", lang[MSG_CANNOT_OPEN_MAIL_ARCHIVE], mbox );
		progerr( errmsg );
	}
	...
	if ( set_append )
	{
		...
		else if ( trio_snprintf( filename, sizeof( filename ), "%s%s", dir, set_append_filename ? set_append_filename + 5 : "mbox" ) == sizeof( filename ) )
		{
			progerr( "Can't build mbox filename" );
		}
		if ( !( fpo = fopen( filename, "a" ) ) )
		{
			trio_snprintf( errmsg, sizeof( errmsg ), "%s \"%s\".", lang[MSG_CANNOT_OPEN_MAIL_ARCHIVE], filename );
			progerr( errmsg );
		}
	}
	...
	for( tlist = set_filter_require; tlist != NULL; require_filter_len++, tlist = tlist->next )
		;
	for( tlist = set_filter_require_full_body; tlist != NULL; require_filter_full_len++, tlist = tlist->next )
		;
	...
	int a = 0;
	for( ; fgets( line_buf, MAXLINE, fp ) != NULL; set_txtsuffix ? PushString( &raw_text_buf, line_buf ) : 0 )
	{
#if DEBUG_PARSE
		printf("IN: %s", line);
#endif 
		...
		{
			offset_current_email = ftell( fp ) - strlen( line_buf );
		}
		if ( set_append )
		{
		...
		if ( !is_deleted && inlist_regex_pos( set_filter_out_full_body, line ) != -1 )
		{
		...
		if ( isinheader )
		{
			...
			else if ( line[0] == '\n' || ( line[0] == '\r' && line[1] == '\n' ) )
			{
				...
				for( head = bp; head; head = head->next )
				{
					...
					{
						head->line = mdecodeRFC2047( head->line, strlen( head->line ), charsetsave );
					}

					if ( head->parsedheader || head->attached || !head->header )
					{
						continue;
					}
					...
					if ( !is_deleted && inlist_regex_pos( set_filter_out, head->line ) != -1 )
					{
						is_deleted = FILTERED_OUT;
					}
				...
				for( head = headp; head; head = head->next )
				{
					...
					else if ( !strncasecmp( head->line, "Content-Disposition:", 20 ) )
					{
						...
						while(*ptr && isspace( *ptr ))
							ptr++;
						if ( !strncasecmp( ptr, "attachment;", 11 ) && ( content != CONTENT_IGNORE ) )
						{
							/* signal we want to attach, rather than embeed this MIME
							 attachment */
							if ( inlist( set_ignore_types, "$NONPLAIN" ) || inlist( set_ignore_types, "$BINARY" ) )
...
#endif
						else if ( !strncasecmp( ptr, "inline;", 7 ) && ( content != CONTENT_IGNORE ) && inlinecontent( type ) )
						{
							inline_force = TRUE;
							/* make sure it is binary */
							content = CONTENT_BINARY;
...
#define DISP_IGNORE 3
						...
						if ( content == CONTENT_IGNORE )
							...
							/* don't save this */
							content = CONTENT_IGNORE;
						else if ( textcontent( type ) || ( inlinehtml && !strcasecmp( type, "text/html" ) ) )
						{
							/* text content or text/html follows.
							 */

							if ( set_save_alts && alternativeparser && content == CONTENT_BINARY )
							{
								file_created = MAKE_FILE; /* please make one */
								description = set_alts_text ? set_alts_text : "alternate version of message";
						...
						else if ( !strncasecmp( type, "message/rfc822", 14 ) )
						{
							 ...
							 * be used.
							 */
							bp = addbody( bp, &lp, "<p><strong>attached mail follows:</strong></p><hr />", BODY_HTMLIZED | bodyflags );
						...
						else
						{
							...
							if ( boundary_id )
							{
								...
								if ( boundary_id )
								{
									...
									if ( '\"' == *boundary_id )
									{
										sscanf( ++boundary_id, "%255[^\"]", boundbuffer );
									}
									else
								...
								while(fgets( line_buf, MAXLINE, fp ))
								{
									...
									if ( !strncmp( line_buf + set_ietf_mbox, "--", 2 )
											&& !strncmp( line_buf + set_ietf_mbox + 2, boundbuffer, strlen( boundbuffer ) ) )
									{
										break;
									}
									if ( !strncasecmp( line_buf, "From ", 5 ) )
								...
								if ( !strncmp( line_buf + set_ietf_mbox + 2 + strlen( boundary_id ), "--", 2 ) && bp != origbp )
								{
					...
					else if ( !strncasecmp( head->line, "Content-Transfer-Encoding:", 26 ) )
					{
						...
						else if ( !strncasecmp( ptr, "x-uue", 5 ) )
						{
							...
							if ( !do_uudecode( fp, line, line_buf, &raw_text_buf, fpo ) )
								break;
						}
						else
						{
							...
							sscanf( ptr, "%63s", code );
							snprintf( line, sizeof( line_buf ) - set_ietf_mbox, " ('%s' %s)\n", code, lang[MSG_ENCODING_IS_NOT_SUPPORTED] );
		...
		else
		{
			 ...
			 */
			if ( !readone && !strncmp( line_buf, "From ", 5 ) && ( *( dp = getfromdate( line ) ) != '\0' ) )
			{
				if ( -1 != binfile )
				{
					close( binfile );
				...
				if ( append_bp && append_bp != bp )
				{
					...
					append_bp = addbody( append_bp, &append_lp, "</div>\n", BODY_HTMLIZED | bodyflags );
				...
				if ( set_mbox_shortened && !increment && num == startnum && max_msgnum >= set_startmsgnum )
				{
					emp = hashlookupbymsgid( msgid );
					if ( !emp )
					{
						snprintf( errmsg, sizeof( errmsg ), "Message with msgid '%s' not found in .hm2index", msgid );
						progerr( errmsg );
					}
					num = emp->msgnum;
				...
				if ( set_mbox_shortened )
				{
					...
					{
						if ( strcmp( msgid, emp->msgid ) && !strstr( emp->msgid, "hypermail.dummy" ) )
						{
							snprintf( errmsg, sizeof( errmsg ), "msgid mismatch %s %s", msgid, emp->msgid );
							progerr( errmsg );
						}
					}
				}
				if ( !emp )
					emp = addhash( num, date, namep, emailp, msgid, subject, inreply, fromdate, charset, NULL, NULL, bp );
				/*
			...
			{ /* decode MIME complient gibberish */
				...
				{ /* why only if set_linkquotes? pcm */
				...
				if ( Mime_B )
				{
					if ( boundp && !strncmp( line, "--", 2 ) && !strncmp( line + 2, boundp->line, strlen( boundp->line ) ) )
					{
						...
						else
						{
							...
							else
							{
								...
								{
									bp = addbody( bp, &lp, set_alts_text ? set_alts_text : "<hr />", BODY_HTMLIZED | bodyflags );
								}
							}
#if DEBUG_PARSE
				...
				if ( data )
				{
					if ( ( content == CONTENT_TEXT ) || ( content == CONTENT_HTML ) )
					{
...
#endif
								bp = addbody( bp, &lp, p,
										( content == CONTENT_HTML ? BODY_HTMLIZED : 0 ) | ( multilinenoend ? BODY_CONTINUE : 0 ) | bodyflags );
						...
						{
							bp = addbody( bp, &lp, data, ( content == CONTENT_HTML ? BODY_HTMLIZED : 0 ) | bodyflags );
						}
#if DEBUG_PARSE
						printf("ALIVE?\n");
#endif
						...
						if ( file_created == MAKE_FILE )
						{
							...
							if ( att_dir == NULL )
							{
								...
								trio_asprintf(&att_dir,"%s%c" DIR_PREFIXER "%s",
										...
										message_name (emp))
#else
								trio_asprintf( &att_dir, "%s%c" DIR_PREFIXER "%04d", dir, PATH_SEPARATOR, num );
#endif
								if ( set_increment != -1 )
									check1dir( att_dir );
								...
								if ( set_usemeta && set_increment != -1 )
								{
									...
									trio_asprintf( &meta_dir, "%s%c" META_DIR, att_dir, PATH_SEPARATOR );
									check1dir( meta_dir );
								}
							}
							...
							else
							{
								...
								if ( !attachname[0] || inlist( att_name_list, fname ) )
									trio_asprintf( &binname, "%s%c%02d-%s", att_dir, PATH_SEPARATOR, att_counter, fname );
								else
									trio_asprintf( &binname, "%s%c%s", att_dir, PATH_SEPARATOR, fname );
...
#define OPENBITMASK O_WRONLY | O_CREAT | O_TRUNC
							...
							if ( binname )
							{
								binfile = open( binname, OPENBITMASK, set_filemode );
								int i = 0;
								char *ext = NULL;
								for( i = strlen( binname ); !( ( binname[i] == '.' ) || ( binname[i] == '/' ) || ( i < 0 ) ); i-- )
									;
								if ( i != 0 )
									i++;
								ext = malloc( sizeof(char) * ( strlen( binname + i ) + 1 ) );
								strncpy( ext, binname + i, strlen( binname ) - i + 1 );

								if ( ( strcasecmp( ext, "c" ) == 0 ) || ( strcasecmp( ext, "txt" ) == 0 ) || ( strcasecmp( ext, "doc" ) == 0 )
										|| ( strcasecmp( ext, "docx" ) == 0 ) || ( strcasecmp( ext, "pdf" ) == 0 ) || ( strcasecmp( ext, "odt" ) == 0 )
										|| ( strcasecmp( ext, "sh" ) == 0 ) || ( strcasecmp( ext, "rtf" ) == 0 ) )
									do_save = 0;
								else
									do_save = 1;
								free( ext );
								...
								if ( -1 != binfile )
								{
									...
									if ( set_usemeta )
									{
										...
										*ptr = '\0';
										...
										META_EXTENSION, meta_dir, PATH_SEPARATOR, ptr + 1 );
										*ptr = PATH_SEPARATOR;
										...
										if ( file_ptr )
										{
											...
											{
												if ( charset )
													fprintf( file_ptr, "Content-Type: %s; charset=\"%s\"\n", type, charset );
												else
													fprintf( file_ptr, "Content-Type: %s\n", type );
											}
											fclose( file_ptr );
								...
								if ( strlen( file ) <= 500 )
								{
									...
									if ( description && description[0] != '\0' && hasblack( description ) )
										desc = convchars( description, charset );
									else if ( inline_force || inlinecontent( type ) )
										desc = attachname[0] ? attachname : "picture";
									else
										desc = attachname[0] ? attachname : "stored";
									...
									{
										struct emailinfo e;
										fill_email_dates( &e, date, fromdate, NULL, NULL );
										subdir = msg_subdir( num, set_use_sender_date ? e.date : e.fromdate );
									}

									if ( inline_force || inlinecontent( type ) )
									{
										/* if we know our browsers can show this type of context
										 as-is, we make a <img> tag instead of <a href>! */
										if ( set_inline_addlink )
										{
											char *created_link = createlink( set_attachmentlink, &att_dir[strlen( dir ) + 1], file, num, type );
											trio_snprintf( buffer, sizeof( buffer ),
													"<p><img src=\"%s%s%c%s\" alt=\"%s\" />\n<br />\n(%s %s: <a href=\"%s%s\">%s</a>)</p>\n",
													subdir ? subdir->rel_path_to_top : "", &att_dir[strlen( dir ) + 1], PATH_SEPARATOR, file, desc, type,
													lang[MSG_ATTACHMENT], subdir ? subdir->rel_path_to_top : "", created_link, file );
											free( created_link );
										}
										else
										{
											trio_snprintf( buffer, sizeof( buffer ), "<img src=\"%s%s%c%s\" alt=\"%s\" />\n",
									...
									else
									{
										...
										trio_snprintf( buffer, sizeof( buffer ), "<ul>\n<li>%s %s: <a href=\"%s%s\">%s</a></li>\n</ul>\n", type,
												lang[MSG_ATTACHMENT], subdir ? subdir->rel_path_to_top : "", created_link, desc );
									...
									if ( !append_bp )
										append_bp = addbody( append_bp, &append_lp, "<div>\n", BODY_HTMLIZED | bodyflags );
									append_bp = addbody( append_bp, &append_lp, buffer, BODY_HTMLIZED | bodyflags );
									trio_snprintf( buffer, sizeof( buffer ), "<!-- attachment=\"%.80s\" -->\n", file );
									append_bp = addbody( append_bp, &append_lp, buffer, BODY_HTMLIZED | bodyflags );
								}
							}
...
static void check_expiry ( struct emailinfo *emp )
{
	...
	if ( !emp->is_deleted )
	{
		...
		if ( email_time == -1 )
			email_time = emp->date;
		if ( email_time != -1 && set_delete_older && email_time < convtoyearsecs( set_delete_older ) )
		{
			emp->is_deleted = FILTERED_OLD;
			option = "delete_older";
		}
		if ( email_time != -1 && set_delete_newer && email_time < convtoyearsecs( set_delete_newer ) )
		{
...
int parse_old_html ( int num, struct emailinfo *ep, int parse_body, int do_insert, struct reply **replylist_tmp, int cmp_msgid )
{
	char line[MAXLINE];
	char *name = NULL;
	char *email = NULL;
	...
	if ( set_linkquotes )
	{
		snprintf( inreply_start, sizeof( inreply_start ), "<dfn>%s</dfn>: <a href=\"", lang[MSG_IN_REPLY_TO] );
	}

	/* prepare the name of the file that stores the message */
	if ( set_nonsequential )
		trio_asprintf( &filename, "%s%s%s.%s", set_dir, subdir ? subdir->subdir : "", msgnum_id_table[num], set_htmlsuffix );
	else
		trio_asprintf( &filename, "%s%s%.4d.%s", set_dir, subdir ? subdir->subdir : "", num, set_htmlsuffix );
						...
						while(fgets( line, MAXLINE, fp ))
						{
							...
							if ( line2 )
							{
								...
								if ( ep != NULL && !ep->bodylist->line[0] )
									ep->bodylist = bp;
								if ( 0 )
									fprintf( stderr, "addbody %p %d from %s", bp, !ep->bodylist->line[0], ptr );
								free( ptr );
								if ( set_linkquotes && !inreply )
								{
	...
	{ /* only do this if the input was reliable */
		...
		if ( replylist_tmp == NULL || !do_insert )
			...
			name, email, msgid, subject, inreply, fromdate, charset, isodate, isofromdate, bp );
		if ( cmp_msgid )
		...
		if ( emp != NULL && replylist_tmp != NULL )
		{
			...
			if ( set_linkquotes && reply_msgnum != -1 )
			{
				...
				if ( hashnumlookup( reply_msgnum, &email2 ) )
					*replylist_tmp = addreply2( *replylist_tmp, email2, emp, 0, NULL );
#else
...
static int loadoldheadersfrommessages ( char *dir, int num_from_gdbm )
{
...
#else
			snprintf( errmsg, sizeof( errmsg ), "folder_by_date requires usegdbm option"
	...
	while(num <= max_num)
	{
		...
		num_added += parse_old_html( num, ep0, parse_body, num_from_gdbm == -1, &replylist_tmp, 0 );
...
void fixnextheader ( char *dir, int num, int direction )
{
	...
	if ( ( e3 = neighborlookup( num, direction ) ) != NULL && ( email = neighborlookup( num - 1, 1 ) ) != NULL )
...
void fixreplyheader ( char *dir, int num, int remove_maybes, int max_update )
{
	...
	{
		snprintf( current_maybe_pattern, sizeof( current_maybe_pattern ), "<li><dfn>%s</dfn>: <a href=", lang[MSG_MAYBE_REPLY] );
		snprintf( current_link_maybe_pattern, sizeof( current_maybe_pattern ), "<li><a name=\"replies\" id=\"replies\"></a><dfn>%s</dfn>: <a href=",
				lang[MSG_MAYBE_REPLY] );
		snprintf( current_reply_pattern, sizeof( current_reply_pattern ), "<li><dfn>%s</dfn>: <a href=", lang[MSG_REPLY] );
		snprintf( current_link_reply_pattern, sizeof( current_reply_pattern ), "<li><a name=\"replies\" id=\"replies\"></a><dfn>%s</dfn>: <a href=",
				lang[MSG_REPLY] );
		snprintf( current_nextinthread_pattern, sizeof( current_nextinthread_pattern ), "<li><dfn>%s</dfn>: <a href=", lang[MSG_NEXT_IN_THREAD] );
		snprintf( current_next_pattern, sizeof( current_next_pattern ), "<li>dfn>%s</dfn>: <a href=", lang[MSG_NEXT_MESSAGE] );

		/* backwards compatiblity */
		snprintf( old2_maybe_pattern, sizeof( old2_maybe_pattern ), "<li><strong>%s:</strong> <a href=", lang[MSG_MAYBE_REPLY] );
		snprintf( old2_reply_pattern, sizeof( old2_reply_pattern ), "<li><strong>%s:</strong> <a href=", lang[MSG_REPLY] );
		snprintf( old2_nextinthread_pattern, sizeof( old2_nextinthread_pattern ), "<li><strong>%s:</strong> <a href=", lang[MSG_NEXT_IN_THREAD] );
		snprintf( old2_next_pattern, sizeof( old2_next_pattern ), "<li><strong>%s:</strong> <a href=", lang[MSG_NEXT_MESSAGE] );
	}
	...
	if ( fp )
	{
		...
		while(bp)
		{
			...
			if ( !strncmp( bp->line, "<!-- lreply", 11 ) )
			{
				...
				{
					list_started = TRUE;
					trio_asprintf( &ptr1, "<li><a name=\"replies\" id=\"replies\"></a>"
							"<dfn>%s</dfn>: %s <a href=\"%s\" title=\"%s\">"
							"%s: \"%s\"</a></li>\n", lang[subjmatch ? MSG_MAYBE_REPLY : MSG_REPLY], del_msg, msg_href( email, email2, FALSE ),
					...
					trio_asprintf( &ptr1, "<li><dfn>%s</dfn>: %s <a href=\"%s\" title=\"%s\">"
							"%s: \"%s\"</a></li>\n", lang[subjmatch ? MSG_MAYBE_REPLY : MSG_REPLY], del_msg, msg_href( email, email2, FALSE ),
							lang[MSG_LTITLE_REPLIES],
#ifdef HAVE_ICONV
							numname, ptr);
			...
			else if ( !strncmp( bp->line, "<!-- reply", 10 ) )
			{
				...
				ptr = convchars( email->subject, email->charset );
#endif
				trio_asprintf( &ptr1, "<li><strong>%s:</strong>%s %s%s: \"%s\"</a></li>\n", lang[subjmatch ? MSG_MAYBE_REPLY : MSG_REPLY], del_msg,
						msg_href( email, email2, TRUE ),
#ifdef HAVE_ICONV
						numname, ptr);
#else
						email->name, ptr);
			...
			if ( next_in_thread - 1 == replynum
					&& ( strcasestr( bp->line, current_next_pattern ) || strcasestr( bp->line, old2_next_pattern ) || strstr( bp->line, old_next_pattern ) ) )
			{
				bp = bp->next;
				continue; /* line duplicates next in thread; surpress */
			}

			if ( !remove_maybes || strncasecmp( bp->line, current_maybe_pattern, strlen( current_maybe_pattern ) )
					|| strncasecmp( bp->line, current_link_maybe_pattern, strlen( current_link_maybe_pattern ) )
					|| strncasecmp( bp->line, old2_link_maybe_pattern, strlen( old2_link_maybe_pattern ) )
					|| strncasecmp( bp->line, old_maybe_pattern, strlen( old_maybe_pattern ) ) )
				fprintf( fp, "%s", bp->line ); /* not redundant or disproven */
			if ( set_linkquotes
					&& ( strcasestr( bp->line, current_reply_pattern ) || strcasestr( bp->line, current_link_reply_pattern )
							|| strcasestr( bp->line, old2_reply_pattern ) || strcasestr( bp->line, old2_link_reply_pattern )
...
void fixthreadheader ( char *dir, int num, int max_update )
{
	...
	{
		if ( rp->next != NULL && ( rp->next->data && rp->next->data->msgnum == num ) && ( rp->data && rp->msgnum != -1 ) )
		{
	...
	if ( ( fp = fopen( filename, "w+" ) ) != NULL )
	{
		while(bp != NULL)
		{
			...
			/* @@ JK: don't I have the charset here? */fprintf( fp, "%s", bp->line );
