	...
	else
	{
		...
		while ((entry = readdir(dir)))
		{
			...
			if (entry->d_type != DT_DIR && (strcmp(entry->d_name, "input")) != 0)
			{
				...
				free(chname);
				LOG_ZVM("***ZVMLog", "real file name", "s", fmap.realfilename,
						1);
				...
				LOG_ZVM("***ZVMLog", "temp file name", "s", fmap.tempfilename,
						1);
				LOG_ZVM("***ZVMLog", "real file size", "ld", fmap.realfilesize,
						1);
				close(ftmp);
				...
				tempwritebytes2channel = puttext2channel(filteredbuff,
						strlen(filteredbuff), fmap.realfilename, fmap.json,
						fdout);
			}
		}
	}
